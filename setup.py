...
if opt.get('PAIR_MOD') == 'yes':
    if opt.get('USE_PBC') == 'yes':
        pairing_module = Extension('pairing', include_dirs = [path+'utils/'], 
                           ...
                           libraries=['pbc', 'gmp'], runtime_library_dirs=[so_location])
    else:
        ...
        pairing_module = Extension('pairing', include_dirs = [path+'utils/', path+'pairingmath/miracl/'], 
                           ...
                           runtime_library_dirs=[so_location])
...
benchmark_module = Extension('benchmark', sources = [path+'utils/benchmarkmodule.c'], runtime_library_dirs=[so_location])
cryptobase = Extension('cryptobase', sources = [path+'cryptobase/cryptobasemodule.c'], runtime_library_dirs=[so_location])

aes = Extension('AES', sources = [path+'cryptobase/AES.c'], runtime_library_dirs=[so_location])
des  = Extension('DES', include_dirs = [path+'cryptobase/libtom/'], sources = [path+'cryptobase/DES.c'], runtime_library_dirs=[so_location])
des3  = Extension('DES3', include_dirs = [path+'cryptobase/libtom/'], sources = [path+'cryptobase/DES3.c'], runtime_library_dirs=[so_location])
